...
export default function SettingsPanel({ onClose, userProfile, tier, voiceLimit, voiceMinutesUsed, onOpenPlans }: SettingsPanelProps) {
    ...
    const buyExtraMinutes = () => {
        if (!user) return;
        window.location.href = `https://buy.stripe.com/test_eVaeYt5wE3GcaFqfZZ?client_reference_id=${user.id}`;
    };
    ...
    return (
        <div className="fixed inset-0 bg-space-deep/90 backdrop-blur-md z-[100] p-4 sm:p-6 overflow-y-auto custom-scrollbar">
            <div className="min-h-full flex items-center justify-center py-8">
                <motion.div
                    initial={{ opacity: 0, scale: 0.95, y: 20 }}
                    animate={{ opacity: 1, scale: 1, y: 0 }}
                    exit={{ opacity: 0, scale: 0.95, y: 20 }}
                    className="w-full max-w-xl bg-space-surface border border-amber/15 rounded-2xl p-6 md:p-8 shadow-[0_0_50px_rgba(232,168,56,0.05)] relative"
                >
                    <button
                        onClick={onClose}
